...
void add_headers(int, const char *);
void not_found(int);
...
void cat_file(int client, FILE *resource)
{
  ...
  do
  {
    length = fread(buf, 1, sizeof(buf), resource);
...
/**********************************************************************/
void add_headers(int client, const char *filename)
{
  char buf[1024];
  ...
  char *mimetype = get_mimetype(filename);
...
/**********************************************************************/
//将请求的文件发送回浏览器客户端
void serve_file(int client, const char *method, const char *filename)
{
  FILE *resource = NULL;
  int numchars = 1;
  char buf[1024];
...
void cat_directory(int client, const char *directory)
{
  ...
  sprintf(buffer, "<p>Directory: %s</p><table><tr><th>Filename</th></tr>\n",
          directory);
  if (d)
  {
...
void serve_directory(int client, const char *method, const char *directory)
{
